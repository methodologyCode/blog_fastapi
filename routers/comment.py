...
from schemas.comment_schema import CommentBase, CommentDisplay
...
def create(request: CommentBase, db: Session = Depends(get_db),
           current_user: DbUser = Depends(get_current_user)) -> CommentDisplay:
...
def delete(comment_id: int, db: Session = Depends(get_db),
    ...
    return Comment.delete(db, current_user.id, id=comment_id)
